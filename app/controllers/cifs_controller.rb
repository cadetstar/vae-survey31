class CifsController < ApplicationController
  ...
  def index
    ...
    case session[:cifs][:status]
      when 'unsent'
        @cifs = @cifs.where(["sent_at is null and (flagged_until is null or flagged_until < :time_now or (cifs.updated_at > (cifs.flagged_until - INTERVAL '7 days') or clients.updated_at > (cifs.flagged_until - INTERVAL '7 days') or companies.updated_at > (cifs.flagged_until - INTERVAL '7 days')))", :time_now => Time.now])
      when 'flagged'
        @cifs = @cifs.where(["sent_at is null and not (flagged_until is null or flagged_until < :time_now or (cifs.updated_at > (cifs.flagged_until - INTERVAL '7 days') or clients.updated_at > (cifs.flagged_until - INTERVAL '7 days') or companies.updated_at > (cifs.flagged_until - INTERVAL '7 days')))", :time_now => Time.now])
      when 'sent'
